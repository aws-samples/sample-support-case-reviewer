...
- ユーザーが「サポートケースをレビュー」「起票文をレビュー」と依頼した場合
- ユーザーがサポートへの問い合わせ内容を提示した場合
- 「次のサポートケースを...」で始まるリクエスト
- サポートケース、起票、問い合わせ、レビュー、確認、チェック などのキーワードが含まれる場合

**重要:** サポートケースのレビュー依頼を受けた場合は、自分でレビューを行わず、
必ずreview_support_caseツールを呼び出してください。

**出力の扱い:** このツールはAWSの技術的なお問い合わせに関するガイドラインを取得し、レビュー用プロンプト(レビュー評価基準とガイドライン)を返します。
その内容に従ってレビューを実行し、結果をユーザーに提示してください。"""
)
guidelines_fetcher = GuidelinesFetcher()

@mcp.tool()
async def review_support_case(case_content: str) -> str:
    """**サポートケースレビューの専用ツール** - サポートケースの内容をサポートケース起票のガイドラインに基づいてレビューするためのプロンプト(レビュー評価基準とガイドライン)を返します

**このツールを使用すべき場面:**
- ユーザーが「サポートケースをレビュー」と言った場合
- ユーザーが「起票文をレビュー」「起票内容を確認」と言った場合
- ユーザーが「サポートケースの内容を見て」と言った場合
- ユーザーがAWSサポートへの問い合わせ文を提示した場合
- 「次のサポートケースを...」で始まるリクエスト
- サポートケース、起票、問い合わせ、ケース内容、レビュー、確認、チェック などのキーワードが含まれる場合

**INSTRUCTIONS:**
1. このツールはレビュー用プロンプト(レビュー評価基準とガイドライン)を返します
2. 返されたプロンプトに従ってレビューを実行してください
3. レビュー結果をユーザーに提示してください

**重要:** サポートケースのレビュー依頼があった場合は、必ずこのツールを使用してください。
自分でレビューを行わず、このツールの出力を使用してください。

Args:
    case_content: レビュー対象のサポートケース内容
    
Returns:
    ガイドラインに基づいてレビューするためのプロンプト(レビュー評価基準とガイドライン)
"""
    # ガイドラインを取得
...
**重点評価項目**: 質問タイプを判定のうえ、質問タイプや文脈に応じて選択。以下は質問タイプ毎の代表例であり、起票内容に応じて柔軟に判断すること
- **仕様確認**: 最終的に実現したいこと、環境や構成の情報、リージョン、参照ドキュメントURL
- **ベストプラクティス/How-to**: 最終的に実現したいこと、環境や構成の情報、リージョン、制約条件、試した対応策
- **トラブルシューティング**: 障害と判断した状況、最終的に実現したいこと、リージョン、リソースID、発生時刻と継続状況、サービス影響範囲、再現手順、ログ・画面キャプチャ、試した対応策、関連サポートケースID
